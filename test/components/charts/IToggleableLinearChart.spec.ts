...
import LToggleableLinearChart from '~/src/components/charts/LToggleableLinearChart.vue'
...
describe('linearChart component', () => {
  let linearChart: Wrapper<LToggleableLinearChart>

  beforeAll(() => {
    linearChart = mount(LToggleableLinearChart, {
      ...defaultParams
    })
  })
  ...
  it('render list items', async () => {
    ...
    const items = () => linearChart.findAllComponents({ name: 'LToggleableLinearChartLine' })
  ...
  it('test translation object', async () => {
    ...
    expect(linearChart.find('.LToggleableLinearChartLine__expand').text()).toBe('__translation__')
    expect(linearChart.find('.LToggleableLinearChartLine__result__value--first').text()).toBe('__translation__: 16.535.343,00')
    expect(linearChart.find('.LToggleableLinearChartLine__result__value--second').text()).toBe('__translation__: 360')
  })

  it('emit expand list', async () => {
    const expandButton = () => linearChart.find('.LToggleableLinearChartLine__expand')
  ...
  it('respects max length allowed for labels', async () => {
    ...
    await linearChart.vm.$nextTick()
    expect(linearChart.find('.LToggleableLinearChartLine__label').text()).toBe('ASCEN...')
  })
...
describe('linearChart component', () => {
  ...
  it('check if magnify is showed', async () => {
    const items = () => linearChart.findAll('.LToggleableLinearChart__list__item__magnify')
